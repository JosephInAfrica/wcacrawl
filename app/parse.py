...
file=tempdir+'wcaworld.comengmembers.aspcid=78445.txt'
...
def get_namenid(file):
    try:
        name=clean_comapny_name(clean(get_soup(file).find('div',class_='member_name').text))
...
# print (get_namenid(file))

def get_profile_table(file):
    # table=get_table_soup(file).find('div',{'class':'memberprofile_row'})
